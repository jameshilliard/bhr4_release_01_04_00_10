...
#define CS752X_NODE_USE_DSCP_PRIO        "cp_use_dscp_prio"
u32 cs_use_dscp_prio = 1;
...
static int cs_proc_node_use_dscp_prio_read(char *buf, char **start,
		off_t offset, int count, int *eof, void *data)
{
	return read_proc(buf, eof, CS752X_NODE_USE_DSCP_PRIO, cs_use_dscp_prio);
}

static int cs_proc_node_use_dscp_prio_write(struct file *file,
		const char *buffer, unsigned long count, void *data)
{
	return write_proc(buffer, count, CS752X_NODE_USE_DSCP_PRIO, &cs_use_dscp_prio);
}
...
int cs_hw_accel_wireless_init(void)
{
	...
	node = create_proc_entry(CS752X_NODE_USE_DSCP_PRIO, S_IRUGO | S_IWUGO, proc_driver_cs752x_ne);
	if (node) {
		node->read_proc = cs_proc_node_use_dscp_prio_read;
		node->write_proc = cs_proc_node_use_dscp_prio_write;
	} else {
		printk(KERN_ERR "ERROR in creating proc entry (%s)!\n", CS752X_NODE_USE_DSCP_PRIO);
	}
...
void cs_hw_accel_wireless_skb_fixup(struct sk_buff *skb)
{
	...
	if (eth->h_proto == htons(ETH_P_8021Q)) {
		...
		if (!cs_use_dscp_prio) {
			switch (vlan_prio) {
				/*case 0: cs_hw_accel_wireless_ipv4_change_dscp(skb, 0x00); break;*/
				case 1: cs_hw_accel_wireless_ipv4_change_dscp(skb, 0x08); break;
				case 2: cs_hw_accel_wireless_ipv4_change_dscp(skb, 0x10); break;
				case 3: cs_hw_accel_wireless_ipv4_change_dscp(skb, 0x18); break;
